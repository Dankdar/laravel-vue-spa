  ...
  <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0">
    ...
    <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
      <form @submit.prevent="register">
        ...
        <div>
          <label class="block font-medium text-sm text-gray-500" for="fname">
            First Name
          </label>
          <input v-model="data.fname" class="p-2 rounded-md shadow-sm bg-white border border-gray-300 text-gray-400 block mt-1 w-full" id="fname" type="text" name="fname" required="required" autofocus="autofocus">
          <div v-if="errors && errors.fname">
            <p v-for="(error, index) in errors.name" :key="'fname-'+index" class="text-red-500">{{ error }}</p>
          </div>
        </div>
        <br>
        <div>
          <label class="block font-medium text-sm text-gray-500" for="lname">
            Last Name
          </label>
          <input v-model="data.lname" class="p-2 rounded-md shadow-sm bg-white border border-gray-300 text-gray-400 block mt-1 w-full" id="lname" type="text" name="lname" required="required" autofocus="autofocus">
          <div v-if="errors && errors.lname">
            <p v-for="(error, index) in errors.name" :key="'lname-'+index" class="text-red-500">{{ error }}</p>
          </div>
        </div>
        <div class="mt-4">
        ...
        <div class="mt-4">
          <label class="block font-medium text-sm text-gray-500" for="phno">
            phone number
          </label>
          <input v-model="data.phno" class="p-2 rounded-md shadow-sm bg-white border border-gray-300 text-gray-400 block mt-1 w-full" id="phno" type="text" name="phno" required="required" autofocus="autofocus">
          <div v-if="errors && errors.phno">
            <p v-for="(error, index) in errors.phno" :key="'name-'+index" class="text-red-500">{{ error }}</p>
          </div>
        </div>
    ...
    <div class="text-center text-sm mt-4">
        ...
        <router-link class="underline text-gray-200 hover:text-rose-500" :to="{name: 'Login'}">
          Sign in now
        </router-link>
    </div>
  </div>
